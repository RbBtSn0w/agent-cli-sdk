...
class CliJsonDriver(AgentDriver):
    ...
    def __init__(self, executable_path: str, base_args: List[str] = None, session_id: Optional[str] = None):
    ...
    async def start(self) -> None:
        try:
            ...
            raise RuntimeError(f"CLI not found at '{self.executable_path}'")
    ...
    async def chat(self, messages: List[Message]) -> AsyncGenerator[StreamEvent, None]:
        ...
        if process.returncode != 0:
            ...
            yield StreamEvent(type=AgentEvent.ERROR, payload=f"CLI failed: {stderr.decode()}")
    ...
    def _map_event(self, data: Dict[str, Any]) -> List[StreamEvent]:
        ...
        res = []
        t = data.get("type")
        if t == "init": self.session_id = data.get("session_id")
