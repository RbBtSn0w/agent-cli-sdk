...
from ..core.types import StreamEvent, AgentEvent
...
class GeminiDriver(CliJsonDriver):
    ...
    """
    def __init__(self, executable_path: str = "gemini", session_id: Optional[str] = None):
        # We use -o stream-json to get machine-readable output from the CLI
        super().__init__(executable_path, base_args=["-o", "stream-json"], session_id=session_id)
    ...
    def _map_event(self, data: Dict[str, Any]) -> List[StreamEvent]:
        ...
        if t == "init":
            ...
            if data.get("role") == "assistant" and data.get("content"):
                res.append(StreamEvent(type=AgentEvent.CONTENT, payload=data.get("content")))
            ...
            res.append(StreamEvent(type=AgentEvent.THOUGHT, payload=f"Executing built-in tool: {data.get('tool_name')}"))
            res.append(StreamEvent(type=AgentEvent.TOOL_CALL, payload={
            ...
            res.append(StreamEvent(type=AgentEvent.TOOL_RESULT, payload={
                "id": data.get("tool_id"),
